...
export default function ChatInterface() {
    ...
    const { isAuthenticated, token, loginWithPassport } = useAximAuth();
    ...
    if (!isAuthenticated) {
        return (
            <div className="chat-interface flex flex-col h-full bg-slate-900 rounded-lg shadow-xl overflow-hidden border border-slate-700 items-center justify-center">
                <div className="text-center p-8 bg-slate-800 rounded-xl shadow-2xl border border-slate-600 max-w-md w-full">
                    <h2 className="text-2xl font-bold text-white mb-4">Login Required</h2>
                    <p className="text-slate-400 mb-8">You must authenticate via AXiM Passport to access the Onyx Swarm Hub.</p>
                    <button
