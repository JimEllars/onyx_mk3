...
impl MarkdownStreamState {
    ...
    pub fn push(&mut self, renderer: &TerminalRenderer, delta: &str) -> Option<String> {
        ...
        if rendered.contains("[WARN] Schema mismatch, triggering correction prompt") {
            rendered = format!("\x1b[33m{}\x1b[0m", rendered); // yellow for warn
        } else if rendered.contains("[SCHEMA OK]") {
            rendered = format!("\x1b[32m{}\x1b[0m", rendered); // green for ok
        }
    ...
    pub fn flush(&mut self, renderer: &TerminalRenderer) -> Option<String> {
        if self.pending.trim().is_empty() {
            ...
            if rendered.contains("[WARN] Schema mismatch, triggering correction prompt") {
                rendered = format!("\x1b[33m{}\x1b[0m", rendered);
            } else if rendered.contains("[SCHEMA OK]") {
                rendered = format!("\x1b[32m{}\x1b[0m", rendered);
            }
            Some(rendered)
        }
    }
...
if pattern.search(code):
    # have to escape backslashes for re.sub, or use a custom replacer
