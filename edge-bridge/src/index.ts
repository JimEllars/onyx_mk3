...
async function kvWriteWithTimeout<T>(promise: Promise<T>, timeoutMs = 500, status: { degraded: boolean }): Promise<T | null> {
    try {
        const timeout = new Promise<typeof TIMEOUT_SYMBOL>((resolve) => setTimeout(() => resolve(TIMEOUT_SYMBOL), timeoutMs));
...
async function kvReadWithTimeout<T>(promise: Promise<T>, timeoutMs = 500, status: { degraded: boolean }): Promise<T | null> {
    try {
        const timeout = new Promise<typeof TIMEOUT_SYMBOL>((resolve) => setTimeout(() => resolve(TIMEOUT_SYMBOL), timeoutMs));
        const result = await Promise.race([promise, timeout]);
        if (result === TIMEOUT_SYMBOL) {
	...
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		try {
			...
			if (request.method === "GET" && url.pathname === "/health") {
				...
				if (idempotencyKey && env.ONYX_STATE) {
					ctx.waitUntil(kvWriteWithTimeout(env.ONYX_STATE.put(`idem:${idempotencyKey}`, responseBody, { expirationTtl: 86400 }), 500, edgeStatus)); // Keep for 24h
				}
