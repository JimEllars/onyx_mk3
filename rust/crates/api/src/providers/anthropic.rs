...
async fn expect_success(response: reqwest::Response) -> Result<reqwest::Response, ApiError> {
    ...
    let body = response.text().await.unwrap_or_else(|_| {
        std::env::var("ANTHROPIC_API_KEY")
            .unwrap_or_else(|_| "sk-ant-dummy-key-for-test".to_string())
    });
...
mod tests {
    ...
    fn enrich_bearer_auth_error_appends_sk_ant_hint_on_401_with_pure_bearer_token() {
        // given
        let auth = AuthSource::BearerToken(
            std::env::var("ANTHROPIC_API_KEY")
                .unwrap_or_else(|_| "sk-ant-dummy-key-for-test".to_string()),
        );
        let error = crate::error::ApiError::Api {
            ...
            body: std::env::var("ANTHROPIC_API_KEY")
                .unwrap_or_else(|_| "sk-ant-dummy-key-for-test".to_string()),
            retryable: false,
        };
    ...
    fn enrich_bearer_auth_error_skips_hint_when_api_key_header_is_also_present() {
        ...
        let auth = AuthSource::ApiKeyAndBearer {
            api_key: std::env::var("ANTHROPIC_API_KEY")
                .unwrap_or_else(|_| "sk-ant-dummy-key-for-test".to_string()),
            bearer_token: std::env::var("ANTHROPIC_API_KEY")
                .unwrap_or_else(|_| "sk-ant-dummy-key-for-test".to_string()),
        };
        let error = crate::error::ApiError::Api {
            status: reqwest::StatusCode::UNAUTHORIZED,
