...
const GITIGNORE_ENTRIES: [&str; 2] = [".onyx/settings.local.json", ".onyx/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claw_json = cwd.join(".onyx.json");
    artifacts.push(InitArtifact {
        name: ".onyx.json",
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.onyx.json`; reserve `.onyx/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".onyx/"));
        assert!(rendered.contains(".onyx.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".onyx").is_dir());
        assert!(root.join(".onyx.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".onyx.json")).expect("read claw json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".onyx/settings.local.json"));
        assert!(gitignore.contains(".onyx/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".onyx/settings.local.json\n").expect("write gitignore");
        ...
        assert!(second_rendered.contains(".onyx/"));
        assert!(second_rendered.contains(".onyx.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".onyx/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".onyx/sessions/").count(), 1);
