...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    if let Ok(claw_config_home) = env::var("ONYX_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".onyx").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".onyx").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    ...
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".onyx").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set ONYX_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       onyx agents".to_string(),
        "  Sources          .onyx/agents, ~/.onyx/agents, $ONYX_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "onyx agents [list|help]",
            "sources": [".onyx/agents", "~/.onyx/agents", "$ONYX_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       onyx skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $ONYX_CONFIG_HOME/skills or ~/.onyx/skills".to_string(),
        "  Sources          .onyx/skills, .omc/skills, .agents/skills, .codex/skills, .claude/skills, ~/.onyx/skills, ~/.omc/skills, ~/.claude/skills/omc-learned, ~/.codex/skills, ~/.claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "onyx skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$ONYX_CONFIG_HOME/skills or ~/.onyx/skills",
            "sources": [
                ".onyx/skills",
                ".omc/skills",
                ".agents/skills",
...
mod tests {
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(help["usage"]["direct_cli"], "onyx agents [list|help]");
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".onyx").join("skills");
        let legacy_commands = workspace.join(".onyx").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(
            ...
            "onyx skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       onyx agents"));
        assert!(agents_help
            .contains("Sources          .onyx/agents, ~/.onyx/agents, $ONYX_CONFIG_HOME/agents"));
        ...
        assert!(skills_help.contains("Install root     $ONYX_CONFIG_HOME/skills or ~/.onyx/skills"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".onyx")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".onyx").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".onyx").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
