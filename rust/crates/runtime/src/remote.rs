...
pub async fn execute_remote_command(
    ...
    if lower_cmd.contains("rm -rf /")
        ...
        || lower_cmd.contains("dd if=/dev/zero")
    {
        return Err(RuntimeError::new(
            "Command contains blacklisted operations and was blocked.".to_string(),
        ));
    }
    ...
    std::fs::write(&cert_path, ssh_token)
        .map_err(|e| RuntimeError::new(format!("Failed to write ssh token: {e}")))?;

    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mut perms = std::fs::metadata(&cert_path)
    ...
    let output = cmd
        .output()
        .await
        .map_err(|e| RuntimeError::new(format!("SSH failed to execute: {e}")))?;
