...
/// fall back to reading `.onyx-base` from `cwd`.
...
mod tests {
    ...
    fn reads_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".onyx-base"), "abc1234def5678\n").expect("write .onyx-base");
    ...
    fn returns_none_for_empty_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".onyx-base"), "  \n").expect("write empty .onyx-base");
    ...
    fn resolve_expected_base_prefers_flag_over_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".onyx-base"), "from_file\n").expect("write .onyx-base");
    ...
    fn matches_with_claw_base_file_in_real_repo() {
        ...
        init_repo(&root);
        let sha = head_sha(&root);
        fs::write(root.join(".onyx-base"), format!("{sha}\n")).expect("write .onyx-base");
        let source = resolve_expected_base(None, &root);
    ...
    fn diverged_with_claw_base_file_after_new_commit() {
        ...
        init_repo(&root);
        let old_sha = head_sha(&root);
        fs::write(root.join(".onyx-base"), format!("{old_sha}\n")).expect("write .onyx-base");
