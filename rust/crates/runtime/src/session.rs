    ...
    pub fn load_from_path(path: impl AsRef<Path>) -> Result<Self, SessionError> {
        ...
        if let Some(expected_root) = session.workspace_root() {
            ...
            if !actual_canonical.starts_with(&expected_canonical)
                && !expected_canonical.starts_with(&actual_canonical)
            {
...
/// Called by external consumers (e.g. onyxhip) to enumerate sessions for a CWD.
#[allow(dead_code)]
...
mod workspace_sessions_dir_tests {
    ...
    fn workspace_sessions_dir_returns_fingerprinted_path_for_valid_cwd() {
        let tmp = std::env::temp_dir().join("onyx-session-dir-test");
    ...
    fn workspace_sessions_dir_differs_for_different_cwds() {
        let tmp_a = std::env::temp_dir().join("onyx-session-dir-a");
        let tmp_b = std::env::temp_dir().join("onyx-session-dir-b");
