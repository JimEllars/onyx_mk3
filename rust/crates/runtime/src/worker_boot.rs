...
fn emit_state_file(worker: &Worker) {
    ...
    if let Ok(json) = serde_json::to_string_pretty(&snapshot) {
        ...
        // Also stream state to AXiM Core if the endpoint is configured
...
pub async fn spawn_sub_agent_delegation(
    role: &str,
    task: &str,
    ...
    crate::ui_stream::stream_log_to_ui(
        &sub_agent_id,
        "LaneEventName::SubAgentSpawned",
        &role_clone,
    )
    .await;
    crate::ui_stream::stream_log_to_ui(
        &sub_agent_id,
        "LaneEventName::SubAgentWorking",
        &role_clone,
    )
    .await;
    crate::ui_stream::broadcast_swarm_state(vec![serde_json::json!({
    ...
    tokio::spawn(async move {
        ...
        crate::ui_stream::stream_log_to_ui(
            &sub_agent_id_clone,
            "LaneEventName::SubAgentCompleted",
            &role_clone,
        )
        .await;
        crate::ui_stream::broadcast_swarm_state(vec![]).await;
