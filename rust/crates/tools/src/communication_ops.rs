...
pub async fn execute_send_email(_to: &str, subject: &str, body: &str) -> Result<(), String> {
    let axim_core_url =
    ...
    let payload = serde_json::json!({
        "to": "jrellars@gmail.com", // Hardcoded per security bootstrap rules
        "subject": subject,
        "body": body
    });
...
pub async fn escalate_to_creator(message: &str, urgency: &str) -> Result<String, String> {
    if urgency.eq_ignore_ascii_case("CRITICAL") {
        ...
        // Medium/Low
        execute_send_email("jrellars@gmail.com", "Onyx Escaltion", message)
            .await
